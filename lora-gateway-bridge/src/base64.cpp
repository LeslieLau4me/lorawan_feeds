...
string Base64::encode(const std::string &input)
{
    ...
    int         max_len = 4 * size + 1;
    char       *out     = new char[max_len];

    int ret = bin_to_b64((const uint8_t *)in, size, out, max_len);
    if (ret == -1) {
        delete[] out;
        throw runtime_error("Invalid input for encoding");
    }
...
string Base64::decode(const std::string &input)
{
    const char *in      = input.c_str();
    int         size    = input.length();
    int         max_len = size;
    uint8_t    *out     = new uint8_t[max_len];

    int ret = b64_to_bin(in, size, out, max_len);
    if (ret == -1) {
        delete[] out;
        throw runtime_error("Invalid input for decoding");
    }

    string result(reinterpret_cast<char *>(out), ret);
    delete[] out;
    return result;
}
