...
int pthread_cond_timeout_np(pthread_cond_t *cond, pthread_mutex_t *mutex, unsigned msecs)
{
    if (msecs != 0) {
        ...
        for (i = 0; i < msecs; i += t) {
            ...
            if (ret != ETIMEDOUT) {
                if (ret)
                    dbg_time("ret=%d, msecs=%u, t=%u", ret, msecs, t);
                break;
            }
        }
...
#define MAX_DNS 16
    ...
    for (i = 0; i < MAX_DNS; i++) dns_info[i] = NULL;
    ...
    if (dns_fp) {
        ...
        for (i = 0; i < MAX_DNS && dns_info[i]; i++) fprintf(dns_fp, "%s\n", dns_info[i]);
        fclose(dns_fp);
    } else {
        dbg_time("fopen %s fail, errno:%d (%s)", dns_file, errno, strerror(errno));
    }

    for (i = 0; i < MAX_DNS && dns_info[i]; i++) free(dns_info[i]);
}
...
void ql_get_driver_rmnet_info(PROFILE_T *profile, RMNET_INFO *rmnet_info)
{
    ...
    if (rc < 0) {
        if (errno != ENOTSUP)
            dbg_time("ioctl(0x%x, qmap_settings) errno:%d (%s), rc=%d",
                     request,
                     errno,
                     strerror(errno),
                     rc);
...
void no_trunc_strncpy(char *dest, const char *src, size_t dest_size)
{
    ...
    for (i = 0; i < dest_size && *src; i++) { *dest++ = *src++; }
