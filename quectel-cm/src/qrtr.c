...
static uint32_t xport_lookup(int lookup_sock_fd, uint32_t service_id, uint32_t version)
{
    ...
    lookup_arg = (struct server_lookup_args *)malloc(
    ...
    dbg_time("%s: num_entries_found %d for type=%d instance=%d",
             __func__,
             lookup_arg->num_entries_found,
             service_id,
             version);
    num_servers_found = 0;
...
{
    int srv_list[] = {
        QMUX_TYPE_WDS, QMUX_TYPE_NAS, QMUX_TYPE_UIM, QMUX_TYPE_DMS, QMUX_TYPE_WDS_ADMIN
    };
    ...
    while (1) {
        ...
        for (ne = 0; ne < nevents; ne++) {
            ...
            if (fd == qmidevice_control_fd[1]) {
                ...
                rc = qmi_recv(fd,
                              &pResponse->MUXMsg,
                              sizeof(cm_recv_buf) - sizeof(QCQMI_HDR),
                              &sq_node,
                              &sq_port);
                if (debug_qmi)
