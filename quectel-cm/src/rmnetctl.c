...
static int rtrmnet_ctl_init(rmnetctl_hndl_t **hndl, uint16_t *error_code)
{
    ...
    *hndl = (rmnetctl_hndl_t *)malloc(sizeof(rmnetctl_hndl_t));
    ...
    if (bind((*hndl)->netlink_fd, (struct sockaddr *)saddr_ptr, sizeof(struct sockaddr_nl)) < 0) {
...
static int rtrmnet_ctl_newvnd(rmnetctl_hndl_t *hndl,
                              char            *devname,
                              char            *vndname,
                              uint16_t        *error_code,
                              uint8_t          index,
                              ...
                              uint32_t         ul_agg_size)
{
    ...
    req.nl_addr.nlmsg_flags = NLM_F_REQUEST | NLM_F_CREATE | NLM_F_EXCL | NLM_F_ACK;
    ...
    val                = devindex;
    attrinfo           = (struct rtattr *)(((char *)&req) + NLMSG_ALIGN(req.nl_addr.nlmsg_len));
    attrinfo->rta_type = IFLA_LINK;
    attrinfo->rta_len  = RTA_ALIGN(RTA_LENGTH(sizeof(val)));
    memcpy(RTA_DATA(attrinfo), &val, sizeof(val));
    req.nl_addr.nlmsg_len = NLMSG_ALIGN(req.nl_addr.nlmsg_len) + RTA_ALIGN(RTA_LENGTH(sizeof(val)));
    ...
    req.nl_addr.nlmsg_len =
        NLMSG_ALIGN(req.nl_addr.nlmsg_len) + RTA_ALIGN(RTA_LENGTH(strlen(vndname) + 1));

    linkinfo              = (struct rtattr *)(((char *)&req) + NLMSG_ALIGN(req.nl_addr.nlmsg_len));
    linkinfo->rta_type    = IFLA_LINKINFO;
    linkinfo->rta_len     = RTA_ALIGN(RTA_LENGTH(0));
    req.nl_addr.nlmsg_len = NLMSG_ALIGN(req.nl_addr.nlmsg_len) + RTA_ALIGN(RTA_LENGTH(0));
    ...
    req.nl_addr.nlmsg_len =
        NLMSG_ALIGN(req.nl_addr.nlmsg_len) + RTA_ALIGN(RTA_LENGTH(strlen(kind)));

    datainfo              = (struct rtattr *)(((char *)&req) + NLMSG_ALIGN(req.nl_addr.nlmsg_len));
    ...
    req.nl_addr.nlmsg_len = NLMSG_ALIGN(req.nl_addr.nlmsg_len) + RTA_ALIGN(RTA_LENGTH(sizeof(id)));
    ...
    if (flagconfig != 0) {
        ...
        req.nl_addr.nlmsg_len =
            NLMSG_ALIGN(req.nl_addr.nlmsg_len) + RTA_ALIGN(RTA_LENGTH(sizeof(flags)));
    }
    ...
    if (ul_agg_cnt > 1) {
        ...
        req.nl_addr.nlmsg_len =
            NLMSG_ALIGN(req.nl_addr.nlmsg_len) + RTA_ALIGN(RTA_LENGTH(sizeof(agg_params)));
    }
...
int rtrmnet_ctl_create_vnd(char    *devname,
                           char    *vndname,
                           uint8_t  muxid,
                           uint32_t qmap_version,
                           uint32_t ul_agg_cnt,
                           uint32_t ul_agg_size)
{
    ...
    if (return_code == RMNETCTL_SUCCESS) {
        return_code = rtrmnet_ctl_newvnd(
            handle, devname, vndname, &error_code, muxid, flagconfig, ul_agg_cnt, ul_agg_size);
        if (return_code) {
