...
void udhcpc_start(PROFILE_T *profile)
{
    ...
    if (profile->ipv4.Address) {
        ...
        if_set_network_v4(ifname,
                          ntohl(profile->ipv4.Address),
    ...
    if (profile->ipv6.Address[0] && profile->ipv6.PrefixLengthIPAddr) {
        ...
        if_set_network_v6(ifname,
                          profile->ipv6.Address,
                          profile->ipv6.PrefixLengthIPAddr,
                          profile->ipv6.Gateway,
                          profile->ipv6.DnsPrimary,
                          profile->ipv6.DnsSecondary);
    }
}
