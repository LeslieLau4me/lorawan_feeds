...
atc_proxy_loop_exit:
    ...
    while (!qlist_empty(&atc_proxy_connection)) {
        ATC_PROXY_CONNECTION *atc_con =
            qnode_to_item(qlist_head(&atc_proxy_connection), ATC_PROXY_CONNECTION, qnode);
...
static void usage(void)
{
    dprintf(" -d <device_name>                      A valid atc device\n"
            "                                       default /dev/ttyUSB2, but /dev/ttyUSB2 may be "
            "invalid\n"
...
int main(int argc, char *argv[])
{
    ...
    while (atc_proxy_quit == 0) {
        if (access(atc_dev, R_OK | W_OK)) {
            dprintf(
                "Fail to access %s, errno: %d (%s). continue\n", atc_dev, errno, strerror(errno));
            // wait device
            sleep(3);
            continue;
        }
        ...
        if (modem_reset_flag) {
            ...
            while (time_to_wait) { time_to_wait = sleep(time_to_wait); }
            modem_reset_flag = 0;
        }
    }
