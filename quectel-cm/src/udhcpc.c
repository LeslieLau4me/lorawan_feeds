...
static void ifc_set_state(const char *ifname, int state)
{
    ...
    if (!access("/sbin/ip", X_OK)) {
        snprintf(
            shell_cmd, sizeof(shell_cmd), "ip link set dev %s %s", ifname, state ? "up" : "down");
    } else {
...
static int ql_raw_ip_mode_check(const char *ifname, uint32_t ip)
{
    ...
    if (fd < 0) {
        dbg_time("%s %d fail to open(%s), errno:%d (%s)",
                 __FILE__,
                 __LINE__,
                 raw_ip,
                 errno,
...
static const char *ipv6Str(const UCHAR Address[16])
{
    ...
    uint16_t    ip[8];
    int         i;
    for (i = 0; i < 8; i++) { ip[i] = (Address[i * 2] << 8) + Address[i * 2 + 1]; }
...
void update_ipv4_address(const char *ifname, const char *ip, const char *gw, unsigned prefix)
{
    ...
    if (!access("/sbin/ip", X_OK)) {
        ...
        snprintf(
            shell_cmd, sizeof(shell_cmd), "ip -%d address add %s/%u dev %s", 4, ip, prefix, ifname);
        ql_system(shell_cmd);

        // ping6 www.qq.com
        snprintf(
            shell_cmd, sizeof(shell_cmd), "ip -%d route add default via %s dev %s", 4, gw, ifname);
        ql_system(shell_cmd);
    } else {
...
void update_ipv6_address(const char *ifname, const char *ip, const char *gw, unsigned prefix)
{
    ...
    if (!access("/sbin/ip", X_OK)) {
        ...
        snprintf(
            shell_cmd, sizeof(shell_cmd), "ip -%d address add %s/%u dev %s", 6, ip, prefix, ifname);
        ql_system(shell_cmd);
...
static void ql_openwrt_setup_wan6(const char *ifname, const IPV6_T *ipv6)
{
    ...
    if (first_ifup)
        ...
        ql_openwrt_system(
            "/etc/init.d/network restart"); // make PC to release old IPV6 address, and RS new IPV6 address
    ...
    if (openwrt_lan) {
        ...
        for (i = 0; i < (ipv6->PrefixLengthIPAddr / 8); i++) Address[i] = ipv6->Address[i];

        snprintf(shell_cmd,
                 sizeof(shell_cmd),
                 "ip route del %s/%u dev %s",
                 ipv6Str(Address),
                 ipv6->PrefixLengthIPAddr,
                 ifname);
...
// use the same policy for all modules
    ...
    if (profile->no_dhcp ||
        profile->request_ops == &mbim_request_ops) { // lots of mbim modem do not support DHCP
    ...
    /* Do DHCP using busybox tools */
    {
        ...
        if (access("/usr/share/udhcpc/default.script", X_OK) &&
            ...
            dbg_time("No default.script found, it should be in '/usr/share/udhcpc/' or "
...
#if 1 // for OpenWrt
        if (!access("/lib/netifd/dhcp.script", X_OK) && !access("/sbin/ifup", X_OK) &&
...
#ifdef USE_DHCLIENT
        pthread_create(&udhcpc_thread_id,
                       &udhcpc_thread_attr,
                       udhcpc_thread_function,
        ...
        if (profile->request_ops == &atc_request_ops) {
            ...
            if (profile->udhcpc_ip != profile->ipv4.Address) {
                ...
                dbg_time("ERROR: IP from udhcpc (%d.%d.%d.%d) is different to IP from ATC "
                         "(%d.%d.%d.%d)!",
                         l[0],
                         l[1],
                         l[2],
                         ...
                         r[2],
                         r[3]);
                ql_get_netcard_carrier_state(
                    ifname); // miss udhcpc default.script or modem not report usb-net-cdc-linkup
            }
        }
        ...
        if (ql_raw_ip_mode_check(ifname, profile->ipv4.Address)) {
            pthread_create(
                &udhcpc_thread_id, NULL, udhcpc_thread_function, (void *)strdup(udhcpc_cmd));
            pthread_join(udhcpc_thread_id, NULL);
        }
        ...
        snprintf(
            rt_cmd, sizeof(rt_cmd), "route -n | grep %s | awk '{print $1}' | grep 0.0.0.0", ifname);
    ...
    if (profile->ipv6.Address[0] && profile->ipv6.PrefixLengthIPAddr) {
        ...
        if (profile->ipv6.DnsPrimary[0] || profile->ipv6.DnsSecondary[0]) {
            ...
            update_resolv_conf(6,
                               ifname,
                               profile->ipv6.DnsPrimary[0] ? dns1str : NULL,
