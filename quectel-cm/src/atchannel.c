...
static const char *s_finalResponsesError[] = {
    ...
    "NO ANSWER", "NO DIALTONE", "COMMAND NOT SUPPORT",
};
 ...
 */
static const char *s_smsUnsoliciteds[] = { "+CMT:", "+CDS:", "+CBM:", "+CMTI:" };
...
static const char *readline()
{
    ...
    while (p_eol == NULL) {
        ...
        do {
            ...
            count = (s_fd == -1) ? 0 : read(s_fd, p_read, MAX_AT_RESPONSE - (p_read - s_ATBuffer));
 ...
 * timeoutMsec == 0 means infinite timeout
 */
static int at_send_command_full(const char   *command,
                                ATCommandType type,
                                const char   *responsePrefix,
                                const char   *smspdu,
                                long long     timeoutMsec,
                                ATResponse  **pp_outResponse)
{
    ...
    err = at_send_command_full_nolock(
        command, type, responsePrefix, smspdu, timeoutMsec, pp_outResponse);
                        ...
                        ATResponse **pp_outResponse)
{
    ...
    err = at_send_command_full(command, SINGLELINE, responsePrefix, pdu, 0, pp_outResponse);
                              ...
                              ATResponse **pp_outResponse)
{
    ...
    err = at_send_command_full(command, MULTILINE, responsePrefix, NULL, 0, pp_outResponse);
...
int at_handshake()
{
    ...
    for (i = 0; i < HANDSHAKE_RETRY_COUNT; i++) {
        ...
        err = at_send_command_full_nolock(
            "ATE0Q0V1", NO_RESULT, NULL, NULL, HANDSHAKE_TIMEOUT_MSEC, NULL);
