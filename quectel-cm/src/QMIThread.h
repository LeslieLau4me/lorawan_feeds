...
#if (defined(CONFIG_REG_QOS_IND) || defined(CONFIG_GET_QOS_INFO) || \
     defined(CONFIG_GET_QOS_DATA_RATE))
...
extern int QmiThreadSendQMITimeout(PQCQMIMSG   pRequest,
                                   PQCQMIMSG  *ppResponse,
                                   unsigned    msecs,
                                   const char *funcname);
#define QmiThreadSendQMI(pRequest, ppResponse) \
    QmiThreadSendQMITimeout(pRequest, ppResponse, 30 * 1000, __func__)
...
struct request_ops {
    ...
    int (*requestSetProfile)(
        PROFILE_T *profile); // 1 ~ success and apn change, 0 ~ success and no apn change, -1 ~ fail
...
extern BOOL
    qmidevice_detect(char *qmichannel, char *usbnet_adapter, unsigned bufsize, PROFILE_T *profile);
...
#ifdef CONFIG_QRTR
extern int rtrmnet_ctl_create_vnd(char    *devname,
                                  char    *vndname,
                                  uint8_t  muxid,
