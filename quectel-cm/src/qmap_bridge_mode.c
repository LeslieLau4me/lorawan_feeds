...
static size_t ql_fread(const char *filename, void *buf, size_t size)
{
    ...
    if (fp) {
        ...
        if (n <= 0 || n == size) {
            dbg_time("warnning: fail to fread(%s), fread=%zu, buf_size=%zu: (%s)",
                     filename,
                     n,
                     size,
                     strerror(errno));
...
static size_t ql_fwrite(const char *filename, const void *buf, size_t size)
{
    ...
    if (fp) {
        ...
        if (n != size) {
            dbg_time("warnning: fail to fwrite(%s), fwrite=%zu, buf_size=%zu: (%s)",
                     filename,
                     n,
                     size,
                     strerror(errno));
...
{
    const char *ifname =
        profile->qmapnet_adapter[0] ? profile->qmapnet_adapter : profile->usbnet_adapter;
    const char *driver;
...
void ql_set_driver_bridge_mode(PROFILE_T *profile)
{
    ...
    if (profile->qmap_mode)
        snprintf(
            filename, sizeof(filename), "/sys/class/net/%s/bridge_mode", profile->qmapnet_adapter);
    else
        snprintf(
            filename, sizeof(filename), "/sys/class/net/%s/bridge_mode", profile->usbnet_adapter);
...
static int ql_qmi_qmap_mode_detect(PROFILE_T *profile)
{
    ...
    if (profile->rmnet_info.size) {
        ...
        if (profile->qmap_mode) {
            ...
            strncpy(profile->qmapnet_adapter,
                    profile->rmnet_info.ifname[offset_id],
                    sizeof(profile->qmapnet_adapter) - 1);
    ...
    snprintf(
        pl->filename, sizeof(pl->filename), "/sys/class/net/%s/qmap_mode", profile->usbnet_adapter);
    if (access(pl->filename, R_OK)) {
        ...
        snprintf(pl->filename,
                 sizeof(pl->filename),
                 "/sys/module/%s/parameters/qmap_mode",
                 profile->driver_name);
        if (access(pl->filename, R_OK)) {
            if (errno != ENOENT) {
                dbg_time("fail to access %s, errno: %d (%s)", pl->filename, errno, strerror(errno));
            ...
            snprintf(pl->filename,
                     sizeof(pl->filename),
                     "/sys/class/net/%s/device/driver/module/parameters/qmap_mode",
                     profile->usbnet_adapter);
            if (access(pl->filename, R_OK)) {
                if (errno != ENOENT) {
                    dbg_time(
                        "fail to access %s, errno: %d (%s)", pl->filename, errno, strerror(errno));
                    goto _out;
                }
            }
        }
    ...
    if (!access(pl->filename, R_OK)) {
        ...
        if (n > 0) {
            ...
            if (profile->qmap_mode > 1) {
                ...
                snprintf(profile->qmapnet_adapter,
                         sizeof(profile->qmapnet_adapter),
                         "%.16s.%d",
                         profile->usbnet_adapter,
                         profile->muxid - 0x80);
            }
...
#else
            dbg_time("File:%s Line:%d Please make sure add next patch to qmi_wwan.c",
                     __func__,
                     __LINE__);
/*
            ...
            */
            ...
            if (profile->qmap_mode > 1) {
                ...
                if (!profile->muxid) {
                    ...
                    profile->muxid = (buf[5 * (profile->muxid - 0x81) + 2] - '0') * 16 +
                                     (buf[5 * (profile->muxid - 0x81) + 3] - '0');
                    snprintf(profile->qmapnet_adapter,
                             sizeof(profile->qmapnet_adapter),
                             "qmimux%d",
                             profile->muxid - 0x81);
                ...
                snprintf(profile->qmapnet_adapter, sizeof(profile->qmapnet_adapter), "qmimux%d", 0);
            }
        }
    }
...
_out:
    if (profile->qmap_mode) {
        ...
        dbg_time(
            "qmap_mode = %d, qmap_version = %d, qmap_size = %d, muxid = 0x%02x, qmap_netcard = %s",
            profile->qmap_mode,
            profile->qmap_version,
            profile->qmap_size,
            profile->muxid,
            profile->qmapnet_adapter);
    }
...
static int ql_mbim_usb_vlan_mode_detect(PROFILE_T *profile)
{
    ...
    if (!access(tmp, F_OK)) {
        ...
        no_trunc_strncpy(profile->qmapnet_adapter,
                         tmp + strlen("/sys/class/net/"),
                         sizeof(profile->qmapnet_adapter) - 1);
...
int ql_qmap_mode_detect(PROFILE_T *profile)
{
    if (profile->software_interface == SOFTWARE_MBIM) {
        ...
        if (access(tmp, F_OK)) {
            rtrmnet_ctl_create_vnd(profile->usbnet_adapter,
                                   profile->qmapnet_adapter,
                                   profile->muxid,
                                   profile->qmap_version,
                                   11,
