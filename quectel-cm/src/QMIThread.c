...
#define qmi_rsp_check_and_return()                                             \
    do {                                                                       \
        ...
        if (le16_to_cpu(pMUXMsg->QMUXMsgHdrResp.QMUXResult) ||                 \
            ...
            dbg_time("%s QMUXResult = 0x%x, QMUXError = 0x%x",                 \
                     __func__,                                                 \
...
PQMI_TLV_HDR GetTLV(PQCQMUX_MSG_HDR pQMUXMsgHdr, int TLVType)
{
    ...
    while (Length >= sizeof(QMI_TLV_HDR)) {
        ...
        pTLVHdr = (PQMI_TLV_HDR)(((UCHAR *)pTLVHdr) + le16_to_cpu(pTLVHdr->TLVLength) +
                                 sizeof(QMI_TLV_HDR));
    }
...
static PQCQMIMSG
ComposeQMUXMsg(UCHAR QMIType, USHORT Type, CUSTOMQMUX customQmuxMsgFunction, void *arg)
{
    ...
    if (customQmuxMsgFunction)
        pRequest->MUXMsg.QMUXMsgHdr.Length =
            cpu_to_le16(customQmuxMsgFunction(&pRequest->MUXMsg, arg) - sizeof(QCQMUX_MSG_HDR));
    else
        pRequest->MUXMsg.QMUXMsgHdr.Length = cpu_to_le16(0x0000);

    pRequest->QMIHdr.Length =
        cpu_to_le16(le16_to_cpu(pRequest->MUXMsg.QMUXMsgHdr.Length) + sizeof(QCQMUX_MSG_HDR) +
                    sizeof(QCQMUX_HDR) + sizeof(QCQMI_HDR) - 1);
...
static USHORT WdsStartNwInterfaceReq(PQMUX_MSG pMUXMsg, void *arg)
{
    ...
    if (profile->profile_index &&
        !s_is_cdma) { //cdma only support one pdp, so no need to set profile index
...
static USHORT WdaSetDataFormat(PQMUX_MSG pMUXMsg, void *arg)
{
    ...
    if (qmap_settings->rx_urb_size == 0) {
        ...
        return sizeof(QCQMUX_MSG_HDR) + sizeof(*pWdsAdminQosTlv) + sizeof(*linkProto) +
               sizeof(*dlTlp);
    } else {
        ...
        pMUXMsg->SetDataFormatReq.DownlinkDataAggregationProtocolTlv.Value =
            cpu_to_le32(qmap_version); //DL QMAP is enabled
        ...
        pMUXMsg->SetDataFormatReq.DlMinimumPassingTlv.Value =
            cpu_to_le32(qmap_settings->dl_minimum_padding);
        ...
        pMUXMsg->SetDataFormatReq.UplinkDataAggregationMaxDatagramsTlv.Value =
            cpu_to_le32(qmap_settings->ul_data_aggregation_max_datagrams);
        ...
        pMUXMsg->SetDataFormatReq.UplinkDataAggregationMaxSizeTlv.Value =
            cpu_to_le32(qmap_settings->ul_data_aggregation_max_size);
#endif
...
static USHORT UimReadTransparentIMSIReqSend(PQMUX_MSG pMUXMsg, void *arg)
{
    ...
    pReadTransparent =
        (PREAD_TRANSPARENT_TLV)(&pMUXMsg->UIMUIMReadTransparentReq
    ...
    return (sizeof(QMIUIM_READ_TRANSPARENT_REQ_MSG) + pMUXMsg->UIMUIMReadTransparentReq.path_len +
...
{
    PROFILE_T *profile = (PROFILE_T *)arg;
    pMUXMsg->CreatetProfileSettingsReq.Length =
...
int QmiThreadSendQMITimeout(PQCQMIMSG   pRequest,
                            PQCQMIMSG  *ppResponse,
                            unsigned    msecs,
                            const char *funcname)
{
    int ret;
...
void QmiThreadRecvQMI(PQCQMIMSG pResponse)
{
    ...
    if (s_pRequest && is_response(s_pRequest, pResponse)) {
        ...
        pthread_cond_signal(&cm_command_cond);
               ...
               (le16_to_cpu(pResponse->CTLMsg.QMICTLMsgHdrRsp.QMICTLType ==
                            QMICTL_REVOKE_CLIENT_ID_IND))) {
        ...
        qmidevice_send_event_to_main(RIL_UNSOL_RESPONSE_VOICE_NETWORK_STATE_CHANGED);
               ...
               (le16_to_cpu(pResponse->MUXMsg.QMUXMsgHdrResp.Type) ==
                QMI_WDA_SET_LOOPBACK_CONFIG_IND)) {
        qmidevice_send_event_to_main_ext(RIL_UNSOL_LOOPBACK_CONFIG_IND,
                                         &pResponse->MUXMsg.SetLoopBackInd,
                                         sizeof(pResponse->MUXMsg.SetLoopBackInd));
    }
        ...
        if (qos_id != 0 && new_flow == 1)
            qmidevice_send_event_to_main_ext(
                RIL_UNSOL_GLOBAL_QOS_FLOW_IND_QOS_ID, &qos_id, sizeof(qos_id));
...
static int requestGetCoexWWANState(void)
{
    ...
    pLteBand =
        (PQMI_COEX_GET_WWAN_STATE_RESP_MSG_LTE_BAND)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x10);

    if (pLteBand && memcmp(pLteBand, &oldLteBand, sizeof(oldLteBand))) {
        oldLteBand = *pLteBand;
        dbg_time("%s ul_freq %d ul_bandwidth %d",
                 __func__,
                 le32_to_cpu(pLteBand->ul_band.freq),
                 le32_to_cpu(pLteBand->ul_band.bandwidth));
        dbg_time("%s dl_freq %d dl_bandwidth %d",
                 __func__,
                 le32_to_cpu(pLteBand->dl_band.freq),
                 le32_to_cpu(pLteBand->dl_band.bandwidth));
    }
    free(pResponse);
    return 0;
}
...
static int requestSetEthMode(PROFILE_T *profile)
{
    ...
    pRequest = ComposeQMUXMsg(QMUX_TYPE_WDS_ADMIN,
                              QMIWDS_ADMIN_SET_DATA_FORMAT_REQ,
                              WdaSetDataFormat,
                              (void *)&qmap_settings);
    err      = QmiThreadSendQMI(pRequest, &pResponse);
    ...
    if (linkProto != NULL && profile->qmap_mode) {
        ...
        dbg_time("qmap_settings.rx_urb_size = %u",
                 qmap_settings
                     .rx_urb_size); //must same as rx_urb_size defined in GobiNet&qmi_wwan driver
    }
    ...
    if (qmap_settings.ul_data_aggregation_max_datagrams) {
        ...
        if (linkProto != NULL) {
            qmap_settings.ul_data_aggregation_max_datagrams =
                MIN(qmap_settings.ul_data_aggregation_max_datagrams, le32_to_cpu(linkProto->Value));
            dbg_time("qmap_settings.ul_data_aggregation_max_datagrams  = %u",
                     qmap_settings.ul_data_aggregation_max_datagrams);
        }
        ...
        if (linkProto != NULL) {
            qmap_settings.ul_data_aggregation_max_size =
                MIN(qmap_settings.ul_data_aggregation_max_size, le32_to_cpu(linkProto->Value));
            dbg_time("qmap_settings.ul_data_aggregation_max_size       = %u",
                     qmap_settings.ul_data_aggregation_max_size);
        }
        ...
        if (linkProto != NULL) {
            ...
            dbg_time("qmap_settings.dl_minimum_padding                 = %u",
                     qmap_settings.dl_minimum_padding);
        }
...
skip_WdaSetDataFormat:
    if (profile->enable_ipv4) {
        if (profile->qmapnet_adapter[0]) {
            ...
            pRequest = ComposeQMUXMsg(QMUX_TYPE_WDS,
                                      QMIWDS_BIND_MUX_DATA_PORT_REQ,
                                      WdsSetQMUXBindMuxDataPort,
                                      (void *)&qmap_settings);
            err      = QmiThreadSendQMI(pRequest, &pResponse);
            qmi_rsp_check_and_return();
            if (pResponse)
        ...
        pRequest     = ComposeQMUXMsg(QMUX_TYPE_WDS,
                                  QMIWDS_SET_CLIENT_IP_FAMILY_PREF_REQ,
                                  WdsSetClientIPFamilyPref,
                                  (void *)&IpPreference);
        err          = QmiThreadSendQMI(pRequest, &pResponse);
        if (pResponse)
            free(pResponse);
    ...
    if (profile->enable_ipv6) {
        if (profile->qmapnet_adapter[0]) {
            ...
            pRequest = ComposeQMUXMsg(QMUX_TYPE_WDS_IPV6,
                                      QMIWDS_BIND_MUX_DATA_PORT_REQ,
                                      WdsSetQMUXBindMuxDataPort,
                                      (void *)&qmap_settings);
            err      = QmiThreadSendQMI(pRequest, &pResponse);
    ...
    pRequest = ComposeQMUXMsg(QMUX_TYPE_WDS,
                              QMIWDS_SET_AUTO_CONNECT_REQ,
                              WdsSetAutoConnect,
                              (void *)&autoconnect_setting);
...
{ //RIL_REQUEST_GET_SIM_STATUS
    ...
    *pSIMStatus = SIM_ABSENT;
    if (s_9x07) {
        ...
        if (pCardStatus != NULL) {
            pPINState = (PQMIUIM_PIN_STATE)((PUCHAR)pCardStatus + sizeof(QMIUIM_CARD_STATUS) +
                                            pCardStatus->AIDLength);
        ...
        *pSIMStatus = SIM_ABSENT;
        ...
        } else if ((pResponse->MUXMsg.UIMGetStateResp.UIMState == 0x02) ||
                   (pResponse->MUXMsg.UIMGetStateResp.UIMState == 0xFF)) {
            *pSIMStatus = SIM_ABSENT;
        } else {
            *pSIMStatus = SIM_ABSENT;
        }
...
static int requestEnterSimPin(const char *pPinCode)
{
    ...
    if (s_9x07)
        pRequest = ComposeQMUXMsg(
            QMUX_TYPE_UIM, QMIUIM_VERIFY_PIN_REQ, UimVerifyPinReqSend, (void *)pPinCode);
    else
        pRequest = ComposeQMUXMsg(
            QMUX_TYPE_DMS, QMIDMS_UIM_VERIFY_PIN_REQ, DmsUIMVerifyPinReqSend, (void *)pPinCode);
    err = QmiThreadSendQMI(pRequest, &pResponse);
    qmi_rsp_check_and_return();
...
{ //RIL_REQUEST_GET_IMSI
    ...
    if (s_9x07) {
        pRequest = ComposeQMUXMsg(QMUX_TYPE_UIM,
                                  QMIUIM_READ_TRANSPARENT_REQ,
                                  UimReadTransparentIMSIReqSend,
                                  (void *)"EF_ICCID");
        err      = QmiThreadSendQMI(pRequest, &pResponse);
    } else {
        return 0;
...
static int requestRegistrationState2(UCHAR *pPSAttachedState)
{
    ...
    *pPSAttachedState = 0;
    ...
    pServiceStatusInfo =
        (PSERVICE_STATUS_INFO)(((PCHAR)&pMUXMsg->GetSysInfoResp) + QCQMUX_MSG_HDR_SIZE);
    ...
    while (remainingLen > 0) {
            ...
            case 0x10: // CDMA
                if (pServiceStatusInfo->SrvStatus == 0x02) {
                    DataCapList = WWAN_DATA_CLASS_1XRTT | WWAN_DATA_CLASS_1XEVDO |
                                  WWAN_DATA_CLASS_1XEVDO_REVA | WWAN_DATA_CLASS_1XEVDV |
        ...
        pServiceStatusInfo =
            (PSERVICE_STATUS_INFO)((PCHAR)&pServiceStatusInfo->TLVLength +
    ...
    dbg_time("%s MCC: %d, MNC: %d, PS: %s, DataCap: %s",
             ...
             MobileCountryCode,
             MobileNetworkCode,
             (*pPSAttachedState == 1) ? "Attached" : "Detached",
             pDataCapStr);
...
static int requestQueryDataCall(UCHAR *pConnectionStatus, int curIpFamily)
{
    ...
    *pConnectionStatus = QWDS_PKT_DATA_DISCONNECTED;
    ...
    if (oldConnectionStatus != *pConnectionStatus || debug_qmi) {
        dbg_time("%s %sConnectionStatus: %s",
                 __func__,
...
static int requestSetupDataCall(PROFILE_T *profile, int curIpFamily)
{
    ...
    pRequest             = ComposeQMUXMsg(
        QMIType, QMIWDS_START_NETWORK_INTERFACE_REQ, WdsStartNwInterfaceReq, profile);
...
static int requestDeactivateDefaultPDP(PROFILE_T *profile, int curIpFamily)
{
    ...
    pRequest = ComposeQMUXMsg(
        QMIType, QMIWDS_STOP_NETWORK_INTERFACE_REQ, WdsStopNwInterfaceReq, &curIpFamily);
    err = QmiThreadSendQMI(pRequest, &pResponse);
    qmi_rsp_check_and_return();
...
static int requestGetIPAddress(PROFILE_T *profile, int curIpFamily)
{
    ...
    pRequest =
        ComposeQMUXMsg(QMIType, QMIWDS_GET_RUNTIME_SETTINGS_REQ, WdsGetRuntimeSettingReq, NULL);
    err = QmiThreadSendQMI(pRequest, &pResponse);
    qmi_rsp_check_and_return();

    pPCSCFIpv6Addr = (PQMIWDS_GET_RUNNING_SETTINGS_PCSCF_IPV6_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, 0x2e); // 0x2e - pcscf ipv6 address
    if (pPCSCFIpv6Addr) {
    ...
    pPCSCFIpv4Addr = (PQMIWDS_GET_RUNNING_SETTINGS_PCSCF_IPV4_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, 0x23); // 0x23 - pcscf ipv4 address
    if (pPCSCFIpv4Addr) {
    ...
    pIpv4Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV4_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV4PRIMARYDNS);
    if (pIpv4Addr) {
        pIpv4->DnsPrimary = pIpv4Addr->IPV4Address;
    }

    pIpv4Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV4_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV4SECONDARYDNS);
    if (pIpv4Addr) {
        pIpv4->DnsSecondary = pIpv4Addr->IPV4Address;
    }

    pIpv4Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV4_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV4GATEWAY);
    if (pIpv4Addr) {
        pIpv4->Gateway = pIpv4Addr->IPV4Address;
    }

    pIpv4Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV4_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV4SUBNET);
    if (pIpv4Addr) {
        pIpv4->SubnetMask = pIpv4Addr->IPV4Address;
    }

    pIpv4Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV4_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV4);
    if (pIpv4Addr) {
        pIpv4->Address = pIpv4Addr->IPV4Address;
    }

    pIpv6Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV6_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV6PRIMARYDNS);
    if (pIpv6Addr) {
        memcpy(pIpv6->DnsPrimary, pIpv6Addr->IPV6Address, 16);
    }

    pIpv6Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV6_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV6SECONDARYDNS);
    if (pIpv6Addr) {
        memcpy(pIpv6->DnsSecondary, pIpv6Addr->IPV6Address, 16);
    }

    pIpv6Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV6_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV6GATEWAY);
    if (pIpv6Addr) {
    ...
    pIpv6Addr = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_IPV6_ADDR)GetTLV(
        &pResponse->MUXMsg.QMUXMsgHdr, QMIWDS_GET_RUNTIME_SETTINGS_TLV_TYPE_IPV6);
    if (pIpv6Addr) {
        memcpy(pIpv6->Address, pIpv6Addr->IPV6Address, 16);
        pIpv6->PrefixLengthIPAddr = pIpv6Addr->PrefixLength;
    }

    pMtu = (PQMIWDS_GET_RUNTIME_SETTINGS_TLV_MTU)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr,
...
static int requestSetProfile(PROFILE_T *profile)
{
    ...
    dbg_time("%s[pdp:%d index:%d] %s/%s/%s/%d/%s",
             __func__,
             profile->pdp,
             ...
             profile->password,
             profile->auth,
             ipStr[profile->iptype]);
    if (!profile->profile_index)
        return -1;

    if (!strcmp(profile->old_apn, new_apn) && !strcmp(profile->old_user, new_user) &&
        !strcmp(profile->old_password, new_password) && profile->old_iptype == profile->iptype &&
    ...
    pRequest = ComposeQMUXMsg(
        QMUX_TYPE_WDS, QMIWDS_MODIFY_PROFILE_SETTINGS_REQ, WdsModifyProfileSettingsReq, profile);
    err = QmiThreadSendQMI(pRequest, &pResponse);
    qmi_rsp_check_and_return();
...
_re_check:
    pRequest = ComposeQMUXMsg(
        QMUX_TYPE_WDS, QMIWDS_GET_PROFILE_LIST_REQ, WdsGetProfileListReqSend, profile);
    err         = QmiThreadSendQMI(pRequest, &pResponse);
    s_pResponse = malloc(le16_to_cpu(pResponse->QMIHdr.Length) + 1);
    qmi_rsp_check_and_return();
    ...
    for (int i = 0; i < profile_num; i++) {
        ...
        pRequest = ComposeQMUXMsg(
            QMUX_TYPE_WDS, QMIWDS_GET_PROFILE_SETTINGS_REQ, WdsGetProfileSettingsReqSend, profile);
        err = QmiThreadSendQMI(pRequest, &pResponse);
        qmi_rsp_check_and_return();
        ...
        if (i == profile_num - 1) {
            pRequest = ComposeQMUXMsg(
                QMUX_TYPE_WDS, QMIWDS_CREATE_PROFILE_REQ, WdsCreateProfileSettingsReqSend, profile);
            err = QmiThreadSendQMI(pRequest, &pResponse);
            qmi_rsp_check_and_return();
            free(pResponse);
            goto _re_check;
    ...
    if (pApnName /* && le16_to_cpu(pApnName->TLVLength)*/)
        uchar2char(profile->old_apn,
                   sizeof(profile->old_apn),
                   &pApnName->ApnName,
                   le16_to_cpu(pApnName->TLVLength));
    if (pUserName /*  && pUserName->UserName*/)
        uchar2char(profile->old_user,
                   sizeof(profile->old_user),
                   &pUserName->UserName,
                   le16_to_cpu(pUserName->TLVLength));
    if (pPassWd /*  && le16_to_cpu(pPassWd->TLVLength)*/)
        uchar2char(profile->old_password,
                   sizeof(profile->old_password),
                   &pPassWd->Passwd,
...
static int requestGetSignalInfo(void)
{
    ...
    // CDMA
    {
        PQMINAS_SIG_INFO_CDMA_TLV_MSG ptlv =
            (PQMINAS_SIG_INFO_CDMA_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x10);
        if (ptlv && ptlv->TLVLength) {
            dbg_time("%s CDMA: RSSI %d dBm, ECIO %.1lf dBm",
                     __func__,
                     ptlv->rssi,
                     (-0.5) * (double)ptlv->ecio);
        }
    }

    // HDR
    {
        PQMINAS_SIG_INFO_HDR_TLV_MSG ptlv =
            (PQMINAS_SIG_INFO_HDR_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x11);
        if (ptlv && ptlv->TLVLength) {
            dbg_time("%s HDR: RSSI %d dBm, ECIO %.1lf dBm, IO %d dBm",
                     __func__,
                     ptlv->rssi,
                     (-0.5) * (double)ptlv->ecio,
                     ptlv->io);
    ...
    // GSM
    {
        PQMINAS_SIG_INFO_GSM_TLV_MSG ptlv =
            (PQMINAS_SIG_INFO_GSM_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x12);
        if (ptlv && ptlv->TLVLength) {
    ...
    // WCDMA
    {
        PQMINAS_SIG_INFO_WCDMA_TLV_MSG ptlv =
            (PQMINAS_SIG_INFO_WCDMA_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x13);
        if (ptlv && ptlv->TLVLength) {
            dbg_time("%s WCDMA: RSSI %d dBm, ECIO %.1lf dBm",
                     __func__,
                     ptlv->rssi,
                     (-0.5) * (double)ptlv->ecio);
        }
    ...
    // LTE
    {
        PQMINAS_SIG_INFO_LTE_TLV_MSG ptlv =
            (PQMINAS_SIG_INFO_LTE_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x14);
        if (ptlv && ptlv->TLVLength) {
            dbg_time("%s LTE: RSSI %d dBm, RSRQ %d dB, RSRP %d dBm, SNR %.1lf dB",
                     __func__,
                     ptlv->rssi,
                     ptlv->rsrq,
                     ptlv->rsrp,
    ...
    // TDSCDMA
    {
        PQMINAS_SIG_INFO_TDSCDMA_TLV_MSG ptlv =
            (PQMINAS_SIG_INFO_TDSCDMA_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x15);
        if (ptlv && ptlv->TLVLength) {
    ...
    if (s_5g_type == WWAN_DATA_CLASS_5G_NSA) {
        PQMINAS_SIG_INFO_5G_NSA_TLV_MSG ptlv =
            (PQMINAS_SIG_INFO_5G_NSA_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x17);
        if (ptlv && ptlv->TLVLength) {
            dbg_time("%s 5G_NSA: RSRP %d dBm, SNR %.1lf dB",
                     __func__,
                     ptlv->rsrp,
                     (0.1) * (double)ptlv->snr);
        }
    ...
    if (s_5g_type == WWAN_DATA_CLASS_5G_SA) {
        PQMINAS_SIG_INFO_5G_SA_TLV_MSG ptlv =
            (PQMINAS_SIG_INFO_5G_SA_TLV_MSG)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x18);
        if (ptlv && ptlv->TLVLength) {
...
static int requestBaseBandVersion(PROFILE_T *profile)
{
    ...
    if (revId && le16_to_cpu(revId->TLVLength)) {
        uchar2char(profile->BaseBandVersion,
                   sizeof(profile->BaseBandVersion),
                   &revId->RevisionID,
...
static int requestRadioPower(int state)
{
    ...
    pRequest = ComposeQMUXMsg(
        QMUX_TYPE_DMS, QMIDMS_SET_OPERATING_MODE_REQ, DmsSetOperatingModeReq, &OperatingMode);
    err = QmiThreadSendQMI(pRequest, &pResponse);
    qmi_rsp_check_and_return();
    free(pResponse);
...
static int requestSetLoopBackState(UCHAR loopback_state, ULONG replication_factor)
{
    ...
    dbg_time("%s(loopback_state=%d, replication_factor=%u)",
             __func__,
             loopback_state,
             replication_factor);

    pRequest = ComposeQMUXMsg(
        QMUX_TYPE_WDS_ADMIN, QMI_WDA_SET_LOOPBACK_CONFIG_REQ, WdaSetLoopBackReq, NULL);
...
static int requestRegisterQos(PROFILE_T *profile)
{
    ...
    pRequest = ComposeQMUXMsg(
        QMUX_TYPE_QOS, QMI_QOS_BIND_DATA_PORT_REQ, QosSetBindMuxDataPort, (void *)profile);
    err = QmiThreadSendQMI(pRequest, &pResponse);
...
UCHAR ql_get_qos_info_data_rate(PQCQMIMSG pResponse, void *max_data_rate)
{
    ...
    qos_tx_granted_flow =
        (PQMI_QOS_GET_QOS_INFO_TLV_GRANTED_FLOW)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x11);
    ...
    qos_rx_granted_flow =
        (PQMI_QOS_GET_QOS_INFO_TLV_GRANTED_FLOW)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x12);
    if (qos_rx_granted_flow != NULL) {
        *(ULONG64 *)(max_data_rate + sizeof(ULONG64)) =
            le64_to_cpu(qos_rx_granted_flow->data_rate_max);
        dbg_time("GET_QOS_INFO: rx_data_rate_max=%llu",
                 *(ULONG64 *)(max_data_rate + sizeof(ULONG64)));
    } else
...
static int requestGetQosInfo(PROFILE_T *profile)
{
    ...
    pRequest =
        ComposeQMUXMsg(QMUX_TYPE_QOS, QMI_QOS_GET_QOS_INFO_REQ, QosGetQosInfoReq, (void *)profile);
    err = QmiThreadSendQMI(pRequest, &pResponse);
    qmi_rsp_check_and_return();
    if (pResponse) {
        #ifdef CONFIG_GET_QOS_DATA_RATE
...
UCHAR ql_get_global_qos_flow_ind_qos_id(PQCQMIMSG pResponse, UINT *qos_id)
{
    ...
    qos_flow_state =
        (PQMI_QOS_GLOBAL_QOS_FLOW_TLV_FLOW_STATE)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x01);
    if (qos_flow_state != NULL) {
        if (le32_to_cpu(qos_flow_state->state_change) == QOS_IND_FLOW_STATE_ACTIVATED &&
            qos_flow_state->new_flow == 1) {
            *qos_id = le32_to_cpu(qos_flow_state->qos_id);
            dbg_time("QMI_QOS_GLOBAL_QOS_FLOW_IND: qos_id=%u state=QOS_IND_FLOW_STATE_ACTIVATED",
                     *qos_id);
        }
...
UCHAR ql_get_global_qos_flow_ind_data_rate(PQCQMIMSG pResponse, void *max_data_rate)
{
    ...
    qos_tx_flow_granted =
        (PQMI_QOS_GLOBAL_QOS_FLOW_TLV_FLOW_GRANTED)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x10);
    if (qos_tx_flow_granted != NULL) {
        *(ULONG64 *)(max_data_rate) = le64_to_cpu(qos_tx_flow_granted->data_rate_max);
        dbg_time("QMI_QOS_GLOBAL_QOS_FLOW_IND: tx_data_rate_max=%llu",
                 *(ULONG64 *)(max_data_rate + 0));
    } else
        dbg_time("QMI_QOS_GLOBAL_QOS_FLOW_IND: No qos_tx_flow_granted");
    qos_rx_flow_granted =
        (PQMI_QOS_GLOBAL_QOS_FLOW_TLV_FLOW_GRANTED)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x11);
    if (qos_rx_flow_granted != NULL) {
        *(ULONG64 *)(max_data_rate + sizeof(ULONG64)) =
            le64_to_cpu(qos_rx_flow_granted->data_rate_max);
        dbg_time("QMI_QOS_GLOBAL_QOS_FLOW_IND: rx_data_rate_max=%llu",
                 *(ULONG64 *)(max_data_rate + sizeof(ULONG64)));
    } else
...
static int nas_get_cell_location_info(void)
{
    ...
    {
        NasGetCellLocationNr5gServingCell *ptlv =
            (NasGetCellLocationNr5gServingCell *)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x2F);
        if (ptlv && ptlv->TLVLength) {
    ...
    {
        NasGetCellLocationLteInfoIntrafrequency *ptlv =
            (NasGetCellLocationLteInfoIntrafrequency *)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x13);
        if (ptlv && ptlv->TLVLength) {
            printf("Intrafrequency LTE Info:\n"
                   ...
                   ptlv->global_cell_id,
                   ptlv->absolute_rf_channel_number,
                   earfcn_to_eutra_band_string(ptlv->absolute_rf_channel_number),
    ...
    {
        NasGetCellLocationLteInfoInterfrequency *ptlv =
            (NasGetCellLocationLteInfoInterfrequency *)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x14);
        if (ptlv && ptlv->TLVLength) {
            ...
            for (i = 0; i < ptlv->freqs_len; i++) {
                NasGetCellLocationLteInfoInterfrequencyFrequencyElement *freq =
                    (((void *)ptlv) + off);
                ...
                printf("\tFrequency [%u]:\n"
                       ...
                       i,
                       freq->eutra_absolute_rf_channel_number,
                       earfcn_to_eutra_band_string(freq->eutra_absolute_rf_channel_number),
...
static int nas_get_rf_band_information(void)
{
    ...
    {
        NasGetRfBandInfoList *ptlv =
            (NasGetRfBandInfoList *)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x01);
        if (ptlv && ptlv->TLVLength) {
    ...
    {
        NasGetRfBandInfoExtendedList *ptlv =
            (NasGetRfBandInfoExtendedList *)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x11);
        if (ptlv && ptlv->TLVLength) {
    ...
    {
        NasGetRfBandInfoBandWidthList *ptlv =
            (NasGetRfBandInfoBandWidthList *)GetTLV(&pResponse->MUXMsg.QMUXMsgHdr, 0x12);
        if (ptlv && ptlv->TLVLength) {
            printf("Bandwidth:\n");
            for (i = 0; i < ptlv->num_instances; i++) {
