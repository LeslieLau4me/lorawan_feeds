...
#define dprintf(fmt, args...)                           \
    do {                                                \
        fprintf(stdout, "%s " fmt, get_time(), ##args); \
    } while (0);
#define SYSCHECK(c)                                                                                \
    do {                                                                                           \
        if ((c) < 0) {                                                                             \
...
static void dump_qrtr(void *buf, size_t len, char flag)
{
    ...
    dprintf("%c ver=%d, type=%d(%s), %x,%x -> %x,%x, confirm_rx=%d, size=%u\n",
            ...
            le32toh(hdr->version),
            le32toh(hdr->type),
            ctrl_pkt_strings[le32toh(hdr->type)],
            le32toh(hdr->src_node_id),
            le32toh(hdr->src_port_id),
            le32toh(hdr->dst_node_id),
            le32toh(hdr->dst_port_id),
            le32toh(hdr->confirm_rx),
            le32toh(hdr->size));
}
...
static int qrtr_node_enqueue(const void           *data,
                             size_t                len,
                             int                   type,
...
static void handle_server_change(struct qrtr_hdr_v1 *hdr)
{
    ...
    dprintf("[qrtr] %s  server on %u:%u(%u:%u) -> service %u, instance %x\n",
            ...
            le32toh(pkt->server.node),
            le32toh(pkt->server.port),
            le32toh(hdr->src_node_id),
            le32toh(hdr->src_port_id),
            le32toh(pkt->server.service),
            le32toh(pkt->server.instance));
...
static void
release_qrtr_client_id(QRTR_PROXY_CONNECTION *qrtr_con, uint8_t QMIType, uint8_t ClientId)
{
    ...
    qlist_for_each(con_node, &qrtr_proxy_connection)
    {
        ...
        if (qrtr_con->ClientFd == clientfd) {
            while (!qlist_empty(&qrtr_con->client_qnode)) {
                QRTR_PROXY_CLINET *qrtr_client =
                    qnode_to_item(qlist_head(&qrtr_con->client_qnode), QRTR_PROXY_CLINET, qnode);
...
static void recv_qrtr_from_dev(struct qrtr_hdr_v1 *hdr)
{
    ...
    if (type == QRTR_TYPE_HELLO) {
        ...
        qlist_for_each(con_node, &qrtr_proxy_connection)
        {
            ...
            {
                QRTR_PROXY_CLINET *qrtr_client =
                    qnode_to_item(client_node, QRTR_PROXY_CLINET, qnode);

                if (qrtr_client->node_id == le32toh(hdr->dst_node_id) &&
                    qrtr_client->port_id == le32toh(hdr->dst_port_id)) {
    ...
    if (le16toh(pQMI->QMIHdr.QMIType) == QMUX_TYPE_CTL) {
        if (pQMI->CTLMsg.QMICTLMsgHdr.QMICTLType == QMICTL_SYNC_REQ) {
            ...
            if (pRsp) {
                ...
                pRsp->QMIHdr.Length =
                    htole16(sizeof(pRsp->CTLMsg.GetClientIdRsp) + sizeof(pRsp->QMIHdr) - 1);
                ...
                pRsp->CTLMsg.QMICTLMsgHdrRsp.TransactionId =
                    pQMI->CTLMsg.QMICTLMsgHdr.TransactionId;
                pRsp->CTLMsg.QMICTLMsgHdrRsp.QMICTLType = pQMI->CTLMsg.QMICTLMsgHdr.QMICTLType;
                pRsp->CTLMsg.QMICTLMsgHdrRsp.Length  = htole16(sizeof(pRsp->CTLMsg.GetClientIdRsp) -
                                                              sizeof(pRsp->CTLMsg.QMICTLMsgHdr));
                ...
                pRsp->CTLMsg.QMICTLMsgHdrRsp.QMUXResult =
                    htole16(ClientId ? 0 : QMI_RESULT_FAILURE);
            ...
            PQCQMIMSG pRsp = (PQCQMIMSG)malloc(256);
            release_qrtr_client_id(qrtr_con,
                                   pQMI->CTLMsg.ReleaseClientIdReq.QMIType,
                                   pQMI->CTLMsg.ReleaseClientIdReq.ClientId);

            if (pRsp) {
                pRsp->QMIHdr.IFType = USB_CTL_MSG_TYPE_QMI;
                pRsp->QMIHdr.Length =
                    htole16(sizeof(pRsp->CTLMsg.ReleaseClientIdRsp) + sizeof(pRsp->QMIHdr) - 1);
                pRsp->QMIHdr.CtlFlags = 0x00;
                pRsp->QMIHdr.QMIType  = QMUX_TYPE_CTL;
                pRsp->QMIHdr.ClientId = 0;
                ...
                pRsp->CTLMsg.QMICTLMsgHdrRsp.TransactionId =
                    pQMI->CTLMsg.QMICTLMsgHdr.TransactionId;
                pRsp->CTLMsg.QMICTLMsgHdrRsp.QMICTLType = pQMI->CTLMsg.QMICTLMsgHdr.QMICTLType;
                pRsp->CTLMsg.QMICTLMsgHdrRsp.Length     = htole16(
                    sizeof(pRsp->CTLMsg.ReleaseClientIdRsp) - sizeof(pRsp->CTLMsg.QMICTLMsgHdr));
                pRsp->CTLMsg.QMICTLMsgHdrRsp.TLVType       = QCTLV_TYPE_RESULT_CODE;
                pRsp->CTLMsg.QMICTLMsgHdrRsp.TLVLength     = htole16(4);
            ...
            if (pRsp) {
                pRsp->QMIHdr.IFType = USB_CTL_MSG_TYPE_QMI;
                pRsp->QMIHdr.Length =
                    htole16(sizeof(pRsp->CTLMsg.GetVersionRsp) + sizeof(pRsp->QMIHdr) - 1);
                pRsp->QMIHdr.CtlFlags = 0x00;
                pRsp->QMIHdr.QMIType  = QMUX_TYPE_CTL;
                pRsp->QMIHdr.ClientId = 0;
                ...
                pRsp->CTLMsg.QMICTLMsgHdrRsp.TransactionId =
                    pQMI->CTLMsg.QMICTLMsgHdr.TransactionId;
                pRsp->CTLMsg.QMICTLMsgHdrRsp.QMICTLType = pQMI->CTLMsg.QMICTLMsgHdr.QMICTLType;
                pRsp->CTLMsg.QMICTLMsgHdrRsp.Length =
                    htole16(sizeof(pRsp->CTLMsg.GetVersionRsp) - sizeof(pRsp->CTLMsg.QMICTLMsgHdr));
                pRsp->CTLMsg.QMICTLMsgHdrRsp.TLVType    = QCTLV_TYPE_RESULT_CODE;
                pRsp->CTLMsg.QMICTLMsgHdrRsp.TLVLength  = htole16(4);
                pRsp->CTLMsg.QMICTLMsgHdrRsp.QMUXResult = htole16(0);
                pRsp->CTLMsg.QMICTLMsgHdrRsp.QMUXError  = htole16(0);
        ...
        qlist_for_each(client_node, &qrtr_con->client_qnode)
        {
            ...
            if (pQMI->QMIHdr.QMIType == qrtr_client->QMIType &&
                pQMI->QMIHdr.ClientId == qrtr_client->ClientId) {
                ...
                if (srv && srv->service) {
                    ...
                    qrtr_node_enqueue(&pQMI->MUXMsg,
                                      le16toh(pQMI->QMIHdr.Length) + 1 - sizeof(QCQMI_HDR),
                                      QRTR_TYPE_DATA,
                                      &from,
                                      &to,
                                      0);
...
qrtr_proxy_loop_exit:
    while (!qlist_empty(&qrtr_proxy_connection)) {
        QRTR_PROXY_CONNECTION *qrtr_con =
            qnode_to_item(qlist_head(&qrtr_proxy_connection), QRTR_PROXY_CONNECTION, qnode);
...
static void usage(void)
{
    dprintf(" -d <device_name>                      A valid qrtr device\n"
            "                                       default /dev/mhi_IPCR, but mhi_IPCR may be "
            "invalid\n"
