...
static int GobiNetGetClientID(const char *qcqmi, UCHAR QMIType)
{
    ...
    if (ioctl(ClientId, IOCTL_QMI_GET_SERVICE_FILE, QMIType) != 0) {
        dbg_time(
            "failed to get ClientID for 0x%02x errno: %d (%s)", QMIType, errno, strerror(errno));
        close(ClientId);
        ClientId = 0;
    }
...
static void *GobiNetThread(void *pData)
{
    ...
    #endif
    if (profile->qmap_mode == 0 ||
        profile->loopback_state) { //when QMAP enabled, set data format in GobiNet Driver
