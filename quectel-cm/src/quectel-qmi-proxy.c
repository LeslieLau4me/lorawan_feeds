...
static int merge_qmi_rsp_packet(void *buf, ssize_t *src_size)
{
    ...
    if (le16toh(header->idenity) != MERGE_PACKET_IDENTITY ||
        le16toh(header->version) != MERGE_PACKET_VERSION ||
        le16toh(header->cur_len) > le16toh(header->total_len))
        return -1;
    ...
    memcpy(
        s_QMIPacket.buf + s_QMIPacket.len, buf + sizeof(QMI_MSG_HEADER), le16toh(header->cur_len));
    s_QMIPacket.len += le16toh(header->cur_len);

    if (le16toh(header->cur_len) < MERGE_PACKET_MAX_PAYLOAD_SIZE ||
        s_QMIPacket.len >= le16toh(header->total_len)) {
        ...
        if (qmi_con->ClientFd == clientfd) {
            while (!qlist_empty(&qmi_con->client_qnode)) {
                QMI_PROXY_CLINET *qmi_client =
                    qnode_to_item(qlist_head(&qmi_con->client_qnode), QMI_PROXY_CLINET, qnode);
                ...
                if (clientDisconnect) {
                    ...
                    pQMI->CTLMsg.ReleaseClientIdReq.QMICTLType =
                        htole16(QMICTL_RELEASE_CLIENT_ID_REQ);
        ...
        if (pQMI->CTLMsg.QMICTLMsgHdr.CtlFlags == QMICTL_CTL_FLAG_RSP) {
            if (!qlist_empty(&qmi_proxy_ctl_msg)) {
                QMI_PROXY_MSG *qmi_msg =
                    qnode_to_item(qlist_head(&qmi_proxy_ctl_msg), QMI_PROXY_MSG, qnode);

                if (qmi_msg->qmi[0].CTLMsg.QMICTLMsgHdrRsp.TransactionId !=
                        pQMI->CTLMsg.QMICTLMsgHdrRsp.TransactionId ||
                    qmi_msg->qmi[0].CTLMsg.QMICTLMsgHdrRsp.QMICTLType !=
                        pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType) {
                    dprintf("ERROR: ctl rsp tid:%d, type:%d - ctl req %d, %d\n",
                            pQMI->CTLMsg.QMICTLMsgHdrRsp.TransactionId,
                            pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType,
                            qmi_msg->qmi[0].CTLMsg.QMICTLMsgHdrRsp.TransactionId,
                            qmi_msg->qmi[0].CTLMsg.QMICTLMsgHdrRsp.QMICTLType);
                } else if (qmi_msg->ClientFd == qmi_proxy_server_fd) {
                    if (le16toh(pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType) ==
                        QMICTL_RELEASE_CLIENT_ID_RESP) {
                        dprintf("--- ClientFd=%d QMIType=%d ClientId=%d\n",
                                qmi_proxy_server_fd,
                                pQMI->CTLMsg.ReleaseClientIdRsp.QMIType,
                                pQMI->CTLMsg.ReleaseClientIdRsp.ClientId);
                    }
                } else {
                    qlist_for_each(con_node, &qmi_proxy_connection)
                    {
                        ...
                        if (qmi_con->ClientFd == qmi_msg->ClientFd) {
                            ...
                            } else if (le16toh(pQMI->CTLMsg.QMICTLMsgHdrRsp.QMICTLType) ==
                                       QMICTL_RELEASE_CLIENT_ID_RESP) {
...
static int qmi_proxy_init(unsigned retry)
{
    ...
    for (i = 0; i < retry; i++) {
        ...
        pQMI->QMIHdr.Length = htole16(le16toh(pQMI->CTLMsg.QMICTLMsgHdr.Length) +
...
qmi_proxy_loop_exit:
    while (!qlist_empty(&qmi_proxy_connection)) {
        QMI_PROXY_CONNECTION *qmi_con =
            qnode_to_item(qlist_head(&qmi_proxy_connection), QMI_PROXY_CONNECTION, qnode);
...
static void usage(void)
{
    dprintf(" -d <device_name>                      A valid qmi device\n"
            "                                       default /dev/cdc-wdm0, but cdc-wdm0 may be "
            "invalid\n"
