...
static PQCQMIMSG ComposeQCTLMsg(USHORT QMICTLType, CUSTOMQCTL customQctlMsgFunction, void *arg)
{
    ...
    if (customQctlMsgFunction)
        pRequest->CTLMsg.QMICTLMsgHdr.Length =
            cpu_to_le16(customQctlMsgFunction(&pRequest->CTLMsg, arg) - sizeof(QCQMICTL_MSG_HDR));
    else
        pRequest->CTLMsg.QMICTLMsgHdr.Length = cpu_to_le16(0x0000);

    pRequest->QMIHdr.Length = cpu_to_le16(le16_to_cpu(pRequest->CTLMsg.QMICTLMsgHdr.Length) +
...
static int libqmi_proxy_open(const char *cdc_wdm)
{
    ...
    ret = QmiThreadSendQMI(
        ComposeQCTLMsg(QMI_MESSAGE_CTL_INTERNAL_PROXY_OPEN, CtlLibQmiProxyOpenReq, (void *)cdc_wdm),
        &pResponse);
    if (!ret && pResponse && pResponse->CTLMsg.QMICTLMsgHdrRsp.QMUXResult == 0 &&
...
static int QmiWwanSendQMI(PQCQMIMSG pRequest)
{
    ...
    if (pollfds[0].revents & POLLOUT) {
        ...
        dbg_time("%s poll=%d, revents = 0x%x, errno: %d (%s)",
                 __func__,
                 ret,
                 pollfds[0].revents,
                 errno,
...
static UCHAR QmiWwanGetClientID(UCHAR QMIType)
{
    ...
    QmiThreadSendQMI(ComposeQCTLMsg(QMICTL_GET_CLIENT_ID_REQ, CtlGetClientIdReq, &QMIType),
                     &pResponse);

    if (pResponse) {
        USHORT QMUXResult =
            cpu_to_le16(pResponse->CTLMsg.QMICTLMsgHdrRsp.QMUXResult); // QMI_RESULT_SUCCESS
        USHORT QMUXError =
            cpu_to_le16(pResponse->CTLMsg.QMICTLMsgHdrRsp.QMUXError); // QMI_ERR_INVALID_ARG
...
static int QmiWwanReleaseClientID(QMI_SERVICE_TYPE QMIType, UCHAR ClientId)
{
    ...
    QmiThreadSendQMI(ComposeQCTLMsg(QMICTL_RELEASE_CLIENT_ID_REQ, CtlReleaseClientIdReq, argv),
                     NULL);
    return 0;
}
...
static int QmiWwanInit(PROFILE_T *profile)
{
    ...
    if (!profile->proxy[0]) {
        for (i = 0; i < 10; i++) {
            ret = QmiThreadSendQMITimeout(
                ComposeQCTLMsg(QMICTL_SYNC_REQ, NULL, NULL), NULL, 1 * 1000, __func__);
            if (!ret)
                break;
            sleep(1);
        }
    ...
    if (profile->qmap_mode) {
        if (pResponse) {
            if (pResponse->CTLMsg.QMICTLMsgHdrRsp.QMUXResult == 0 &&
                ...
                for (NumElements = 0; NumElements < pResponse->CTLMsg.GetVersionRsp.NumElements;
                     NumElements++) {
    #if 0
    ...
    #endif
                    if (pResponse->CTLMsg.GetVersionRsp.TypeVersion[NumElements].QMUXType ==
                        QMUX_TYPE_WDS_ADMIN)
                        profile->qmap_version =
                            (pResponse->CTLMsg.GetVersionRsp.TypeVersion[NumElements].MinorVersion >
                             16);
                }
            }
...
static int QmiWwanMergeQmiRsp(void *buf, ssize_t *src_size)
{
    ...
    if (le16_to_cpu(header->idenity) != MERGE_PACKET_IDENTITY ||
        le16_to_cpu(header->version) != MERGE_PACKET_VERSION ||
        le16_to_cpu(header->cur_len) > le16_to_cpu(header->total_len))
        return -1;
    ...
    memcpy(s_QMIPacket.buf + s_QMIPacket.len,
           buf + sizeof(QMI_MSG_HEADER),
           le16_to_cpu(header->cur_len));
    s_QMIPacket.len += le16_to_cpu(header->cur_len);

    if (le16_to_cpu(header->cur_len) < MERGE_PACKET_MAX_PAYLOAD_SIZE ||
        s_QMIPacket.len >= le16_to_cpu(header->total_len)) {
        ...
        for (ne = 0; ne < nevents; ne++) {
            ...
            if (fd == cdc_wdm_fd) {
                ...
                if ((profile->qmap_mode == 0 || profile->qmap_mode == 1) &&
                    QmiWwanMergeQmiRsp(cm_recv_buf, &nreads))
                    continue;
    #endif
