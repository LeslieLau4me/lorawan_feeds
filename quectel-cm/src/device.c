    ...
    while ((ent = readdir(pDir)) != NULL) {
        ...
        if (profile->usb_dev.idVendor == 0x2c7c || profile->usb_dev.idVendor == 0x05c6) {
            dbg_time("Find %s/%s idVendor=0x%x idProduct=0x%x, bus=0x%03x, dev=0x%03x",
                     rootdir,
                     ent->d_name,
                     profile->usb_dev.idVendor,
                     profile->usb_dev.idProduct,
                     profile->usb_dev.busnum,
                     profile->usb_dev.devnum);
        }
        ...
        if (netcard[0] == '\0') { //for centos 2.6.x
            ...
            if (!access(pl->path, F_OK)) {
                snprintf(
                    pl->path, sizeof(pl->path), "%s/%s:1.4/GobiQMI:%s", rootdir, ent->d_name, c);
                if (!access(pl->path, F_OK)) {
        ...
        if (driver_type == SOFTWARE_QMI || driver_type == SOFTWARE_MBIM) {
            ...
            if (atIntf != -1) {
                ...
                if (devname[0] && !strcmp(devname, "tty")) {
                    snprintf(
                        pl->path, sizeof(pl->path), "%s/%s:1.%d/tty", rootdir, ent->d_name, atIntf);
                    dir_get_child(pl->path, devname, sizeof(devname), "tty");
                }
            }
        }
        ...
        while ((ent = readdir(pDir)) != NULL) {
            ...
            if (!strcmp(rootdir, "/sys/bus/mhi/devices")) {
                snprintf(path,
                         ...
                         "%s/%.13s_IPCR",
                         rootdir,
                         ent->d_name); // 13 is sizeof(0306_00.01.00)
                if (!access(path, F_OK)) {
...
int get_driver_type(PROFILE_T *profile)
{
    ...
    dbg_time("%s unknow bInterfaceClass=%d, bInterfaceSubClass=%d",
             __func__,
             profile->usb_intf.bInterfaceClass,
             profile->usb_intf.bInterfaceSubClass);
    return DRV_INVALID;
}
...
int ql_capture_usbmon_log(PROFILE_T *profile, const char *log_path)
{
    ...
    snprintf(usbmon_path,
             sizeof(usbmon_path),
             "/sys/kernel/debug/usb/usbmon/%du",
             profile->usb_dev.busnum);
