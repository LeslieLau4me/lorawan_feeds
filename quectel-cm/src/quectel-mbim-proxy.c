...
static int handle_client_connect(int server_fd)
{
    ...
    if (fcntl(client_fd, F_SETFL, fcntl(client_fd, F_GETFL) | O_NONBLOCK) < 0)
        mbim_debug(
            "fcntl set client(%d) NONBLOCK attribute failed: %s\n", client_fd, strerror(errno));
...
static int handle_client_request(int mbim_dev_fd, int client_fd, void *pdata, int len)
{
    ...
    if (verbose)
        mbim_debug("REQ client_fd=%d, client_idx=%d, tid=%u\n",
                   cm_clients[client_idx].client_fd,
                   cm_clients[client_idx].client_idx,
                   TransactionId);
    ...
    if (TransactionId == 0) {
        ...
        for (i = 0; i < CM_MAX_CLIENT; i++) {
            if (cm_clients[i].client_idx == client_idx && cm_clients[i].client_fd > 0) {
                ...
                if (verbose)
                    mbim_debug("RSP client_fd=%d, client_idx=%d, tid=%u\n",
                               cm_clients[i].client_fd,
                               cm_clients[i].client_idx,
                               TransactionId);
                non_block_write(cm_clients[i].client_fd, pResponse, len);
...
error:
    safe_close(mbim_server_fd);
    for (i = 0; i < CM_MAX_CLIENT; i++) { safe_close(cm_clients[i].client_fd); }
